...
#./buildSybaseDocs $host $port ${MGD_DBSERVER} ${MGD_DBNAME} ${MGD_DBUSER} openup ${MGD_DBPASSWORDFILE}` ${DATALOADSOUTPUT}/mgi/schemaspy-$host
#./buildSybaseDocs $host $port ${MGD_DBSERVER} ${MGD_DBNAME} ${MGD_DBUSER} `cat ${MGD_DBPASSWORDFILE}` ${DATALOADSOUTPUT}/mgi/schemaspy-$host
./buildSybaseDocs rohan 4100 DEV_MGI mgd mgd_public mgdpub ${DATALOADSOUTPUT}/mgi/schemaspy-rohan
